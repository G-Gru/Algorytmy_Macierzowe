...
function persist_results(records, summary; output_dir::AbstractString, max_size::Int, step::Int, trials::Int, random_seed::Int)
    mkpath(output_dir)
    raw_path = joinpath(output_dir, "matrix_inversion_raw.csv")
    ...
    summary_path = joinpath(output_dir, "matrix_inversion_summary.csv")
    ...
    config_path = joinpath(output_dir, "matrix_inversion_config.txt")
...
function plot_results(summary, sizes; output_dir::AbstractString)
    ...
    p_ops = plot(
        ...
        title = "Average operation counts vs size",
        marker = :circle,
        legend = :topleft,
        linewidth = 2
    )
    ...
    p_ops_log = plot(
        ...
        title = "Average operation counts vs size (log-log)",
        marker = :circle,
        legend = :topleft,
        linewidth = 2,
        yscale = :log10
    ...
    p_time_log = plot(
        ...
        title = "Average runtime vs size (log-log)",
        marker = :circle,
        legend = :topleft,
        linewidth = 2,
        yscale = :log10
    ...
    ops_path = joinpath(output_dir, "matrix_inversion_operations_vs_size.png")
    ops_log_path = joinpath(output_dir, "matrix_inversion_operations_vs_size_log.png")
    time_path = joinpath(output_dir, "matrix_inversion_time_vs_size.png")
    time_log_path = joinpath(output_dir, "matrix_inversion_time_vs_size_log.png")
    ...
    for n in sizes
        for trial in 1:trials
            ...
            elapsed = @elapsed begin
                _, _, _ = inverse(A, default_mul, 'N', add_ref, mul_ref)
            end
